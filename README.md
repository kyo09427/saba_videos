...
│   ├── subscriptions/
│   │   ├── subscriptions_screen.dart      # 登録チャンネル動画フィード（3段階レスポンシブ）
│   │   └── subscriptions_channel_list_screen.dart  # 登録チャンネル一覧画面（縦リスト）
│   ├── timeline/
...
└── widgets/                                # 再利用可能なウィジェット
    ...
    ├── app_navigation_scaffold.dart       # PC/モバイル切り替えScaffold（サイドバー/ボトムナビ）
    └── app_mobile_top_bar.dart            # 共通上部バー（ロゴ・通知・検索）スマホ・タブレット用
```
...
- **登録チャンネル画面 レスポンシブ対応強化**
  ...
  - 🟢 **「すべて」ボタンを右端固定**: アイコンリストの右端にグラデーション付き「すべて」ボタンを固定表示し、スクロールしても常にアクセス可能に
  - 🟢 **登録チャンネル一覧画面を新設**: 「すべて」タップで縦リスト形式のチャンネル一覧画面へ遷移。アバター・チャンネル名・bio表示、タップでチャンネルページへ（`subscriptions_channel_list_screen.dart`）
  - 🟢 **タブレット版チャンネル選択サイドバー追加**: 600〜1099px 幅で左に240px幅のチャンネル選択サイドバーを表示。「すべてのチャンネル」ボタン＋縦スクロールチャンネルリスト、右にカテゴリフィルター＋動画コンテンツの2カラム構成
  - 🟢 **3段階レスポンシブレイアウト確立**:
    - スマホ（≤600px）: 上部チャンネルアイコン横スクロール＋縦1列動画リスト
    - タブレット（601〜1099px）: 左チャンネル選択サイドバー＋右動画グリッド
    - PC（≥1100px）: AppNavigationScaffoldのナビゲーションサイドバーで完結（変更なし）
- **共通上部バー（タイムライン・登録チャンネル画面）**
  - 🟢 **`AppMobileTopBar` 共通ウィジェット新設**: ロゴ・キャスト・通知バッジ付きベル・検索アイコンを提供する静的ヘルパークラス（`lib/widgets/app_mobile_top_bar.dart`）
  - 🟢 **タイムライン・登録チャンネル画面に適用**: 「タイムライン」「登録チャンネル」テキストヘッダーをホーム画面と同じ共通上部バーに置き換え
  - 🟢 **PC時のロゴ非表示**: PC（≥1100px）ではサイドバーにロゴがあるためロゴ非表示・アイコンのみ表示。スマホ・タブレットはロゴ＋アイコン両方表示
- **ロゴ表示修正**
  - 🔴 **モバイルロゴをアプリアイコンに統一**: ホーム・タイムライン・登録チャンネル画面のスマホ版ロゴが `Icons.play_circle_filled`（Materialアイコン）になっていた問題を修正。`Image.asset('icon.png')` に統一（PCサイドバーと同一）
